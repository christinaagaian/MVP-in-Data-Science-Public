...
if file_salary and file_salary_old and file_inflation:
    ...
    sectors = sorted(sectors, key=lambda x: ('из них связь' in x.lower(), x))
    ...
    if salary_type == 'Номинальная':
        sns.lineplot(data=plot_data, x='year', y='salary_nominal', marker='o', ax=ax, color=palette[1])
        ax.set_ylabel("Номинальная зарплата (₽)")
    else:
        sns.lineplot(data=plot_data, x='year', y='salary_real', marker='o', ax=ax, color=palette[2])
        ax.set_ylabel("Реальная зарплата (₽, цены 2024 года)")
    
    ax.set_title(f"{salary_type} зарплата: {selected_sector}", fontsize=16)
    ax.set_xlabel("Год")
